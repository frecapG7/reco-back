...
describe("Test getUser", () => {
  ...
  let requestCountStub;
  let recommendationCountStub;

  beforeEach(() => {
    userFindByIdStub = sinon.stub(User, "findById");
    requestCountStub = sinon.stub(Request, "countDocuments");
    recommendationCountStub = sinon.stub(Recommendation, "countDocuments");
  });
  afterEach(() => {
    userFindByIdStub.restore();
    requestCountStub.restore();
    recommendationCountStub.restore();
  });
  ...
  it("Should return user", async () => {
    ...
    requestCountStub.resolves(4);
    recommendationCountStub.resolves(2);

    const result = await userService.getUser("123");
    expect(userFindByIdStub.calledWith("123")).toEqual(true);
    expect(result.requestCount).toEqual(4);
    expect(result.recommendationCount).toEqual(2);
  });
});
